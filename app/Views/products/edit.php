        ...
        <div class="col-md-7">
            <div class="card border-0 shadow-sm p-4">
                <div class="d-flex justify-content-between pb-3">
                    <h2 class="text-uppercase text-dark">Edit Product</h2>
                ...
                <form action="/products/edit/<?= $product['id'] ?>" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="name" class="form-control btn-rounded-none" value="<?= esc($product['name']) ?>" required>
                    </div>
                    <div class="mb-3">
                        <label class="form-label text-uppercase">Description</label>
                        <textarea name="description" class="form-control btn-rounded-none" rows="4"><?= esc($product['description']) ?></textarea>
                    </div>
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label class="form-label text-uppercase">Price ($)</label>
                            <input type="number" name="price" class="form-control btn-rounded-none" step="0.01" min="0" value="<?= $product['price'] ?>" required>
                        </div>
                        <div class="col-md-6 mb-3">
                            <label class="form-label text-uppercase">Stock</label>
                            <input type="number" name="stock" class="form-control btn-rounded-none" min="0" value="<?= $product['stock'] ?>" required>
                        </div>
                    </div>
                    <div class="mb-3">
                        ...
                        <?php if ($product['image']): ?>
                            <div class="pb-2">
                                <img src="/uploads/<?= esc($product['image']) ?>" alt="current image" style="height:80px; object-fit:cover;">
                    ...
                    <div class="d-grid mt-4">
                        <button type="submit" class="btn btn-dark btn-medium text-uppercase btn-rounded-none">Update Product</button>
                    </div>
                </form>
            </div>
        </div>
